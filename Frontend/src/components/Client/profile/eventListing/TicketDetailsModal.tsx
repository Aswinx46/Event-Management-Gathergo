  ...
  const TicketModal: React.FC<TicketModalProps> = ({ open, setIsOpen, ticket }) => {
    ...
    return (
      <Dialog open={open} onOpenChange={(v) => !v && setIsOpen(false)}>
        {open && (
          <DialogContent className="max-w-md w-full bg-gradient-to-br from-white via-purple-50 to-purple-100 shadow-xl border-0 p-0 overflow-hidden">
            <AnimatePresence>
              <motion.div
                key="ticket-modal-anim"
